    ...
    <div class="page-content">
        <div class="row row-cols-1 row-cols-lg-2 row-cols-xl-2">
            <div class="col-xl-8">
                ...
                <!-- <div class="row mb-3" >
                            <label for="input35" class="col-sm-3 col-form-label">Satuan</label>
                            <div class="col-sm-9">
                        ...
                        <tbody class="text-center tableSupplier">
                            <?php $i = 1; foreach ($transaksi as $st): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?= base_url('sibabad/sale/detailTransaksi/') . $st['id_sale'] ?>"
                                        ...
                                        <a href="<?= base_url('sibabad/refund/cetakTransaksi/') . $st['invoice'] ?>"
                                            type="button" class="btn btn-warning btn-sm">
                                            <span class="bx bx-printer"></span>
                                        </a>
                                        <?php else : ?>
                                        <a href="<?= base_url('sibabad/tukar/cetakTransaksi/') . $st['invoice'] ?>"
                                            type="button" class="btn btn-warning btn-sm">
                                            <span class="bx bx-printer"></span>
                                        </a>
                                        <?php endif; ?>
...
<!-- Modal Transaksi Harian -->
<div class="modal fade" id="transaksiharianModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Transaksi Harian</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="input-group mb-3">
                    <label class="input-group-text" for="inputGroupSelect01">Filter</label>
                    <select class="form-select" id="filter-tanggal">
                    ...
                    <button class="btn  btn-primary" id="cetakpdf">
                        Cetak Pdf</button>
                </div>
                                    ...
                                    <tbody>
                                        <?php $i = 1; foreach ($kas_masuk as $kas): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-primary btn-sm btnEditKasMasuk"
                                                        data-kas_kode="<?= $kas['kode'] ?>"
                                                        data-kas_name="<?= $kas['kas_name'] ?>"
                                                        data-kas_tipe="<?= $kas['tipe'] ?>"
                                                        data-kas_total="<?= $kas['kas_total'] ?>"
                                                        data-id_kas="<?= $kas['id_kas'] ?>">
                                                        <span class="bx bx-pencil"></span>
                                                    </button>
                                                    <button type="button" class="btn btn-danger btn-sm btnDeleteKasMasuk"
                                                        data-id_kas="<?= $kas['id_kas'] ?>">
                                                        <span class="bx bx-trash"></span>
                                                    </button>
                                                </td>
                                    ...
                                    <tbody>
                                        <?php $i = 1; foreach ($kas_keluar as $kas): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <button type="button" class="btn btn-primary btn-sm btnEditKasKeluar"
                                                        data-kas_kode="<?= $kas['kode'] ?>"
                                                        data-kas_name="<?= $kas['kas_name'] ?>"
                                                        data-kas_tipe="<?= $kas['tipe'] ?>"
                                                        data-kas_total="<?= $kas['kas_total'] ?>"
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm btnDeleteKasKeluar"
                                                        data-id_kas="<?= $kas['id_kas'] ?>">
                                                        <span class="bx bx-trash"></span>
                                                    </button>
                                                </td>
...
<script type="text/javascript">
    function domo() {
        ...
        $(document).on('keydown', function (e) {
            if (e.key === 'F9') {
                e.preventDefault();
                $('#transaksi_harian').trigger('click');
                return false;
            }
        });
...
<script>
    $(function () {
        if (window.location == `<?= base_url('sibabad/sale/index') ?>`) {
        ...
        function detailKeranjang() {
            let keranjang = ''
            $.ajax({
                url: `<?= base_url('sibabad/sale/keranjang') ?>`,
        ...
        $('#barcode').autocomplete({
            source: `<?= base_url('sibabad/sale/barcode') ?>`,
            autoFocus: true,
            select: function (e, ui) {
                $.ajax({
                    url: `<?= base_url('sibabad/sale/detail') ?>`,
                    type: 'get',
                    data: {
        ...
        $('#jumlah').on('input', function (e) {
            ...
            if (isNaN(jumlah) || jumlah == 0) {
                ...
                $.ajax({
                    url: `<?= base_url('sibabad/sale/cekStok') ?>`,
                    data: {
                        barcode: barcode,
                        iditem: iditem
                    },
        ...
        function tambahKeKranjang() {
            ...
            $.ajax({
                url: `<?= base_url('sibabad/sale/tambahKeranjang') ?>`,
                method: 'post',
                data: {
        ...
        $('.content').on('click', '#hapus-item', function () {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `<?= base_url('sibabad/sale/hapusItem') ?>`,
                        type: 'post',
                        data: {
                            [$('#token').attr('name')]: $('#token').val(),
                            iditem: $(this).data('id'),
        ...
        $('.wrapper').on('click', '#edit-keranjang', function () {
            $.ajax({
                url: `<?= base_url('sibabad/sale/ubahItem') ?>`,
                type: 'post',
                dataType: 'json',
                data: $('form').serialize(),
                success: function (response) {
        ...
        function modal_edit_item() {
            ...
            $.ajax({
                url: `<?= base_url('sibabad/sale/cekHarga') ?>`,
                data: {
                    item_id: item_id,
                    jumlah: jumlah,
                    harga: harga
            ...
            $.ajax({
                url: `<?= base_url('sibabad/sale/cekLevel') ?>`,
                data: {
                    barcode: barcode,
                    level: level,
                    jumlah: jumlah
        ...
        $('.wrapper').on('click', '#batal', function () {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: `<?= base_url('sibabad/sale/hapusTransaksi') ?>`,
                        success: function (response) {
                            detailKeranjang()
                            $('#pelanggan').val('')
                            $('#diskon').val(0)
        ...
        $('.wrapper').on('click', '#bayar_tunai', function () {
            ...
            if (tunai < 1) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `<?= base_url('sibabad/sale/bayarTransaksi') ?>`,
                            type: 'post',
                            dataType: 'json',
                            data: {
                                [$('#token').attr('name')]: $('#token').val(),
                            ...
                            success: function (response) {
                                if (response.status) {
                                    Swal.fire({
                                        ...
                                        if (cetak == '1') {
                                            window.open(
                                                `<?= base_url('sibabad/sale/cetakTransaksi/') ?>${response.idpenjualan}`
                                            );
                                        }
...
<script>
    $(document).ready(function () {
        $('.btnSave').on('click', function () {
            ...
            if (customer_name !== '') {
                $.ajax({
                    url: '<?= base_url('sibabad/customer/saveCustomer') ?>', // URL untuk POST
                    method: 'POST',
                    data: {
                    ...
                    success: function (data) {
                        ...
                        if (data.status === 'success') {
                            ...
                            window.location.href = '<?= base_url('sibabad/sale/index') ?>'
        ...
        $(document).on('click', '.btnDeleteCustomer', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: `<?= base_url('sibabad/customer/deleteCustomer/') ?>${id_customer}`,
                        method: 'DELETE',
                        dataType: 'json',
                        success: function (response) {
                            if (response.success) {
...
<script>
    $(document).ready(function () {
        $('.btnSaveKasMasuk').on('click', function () {
            ...
            if (kas_name !== '') {
                $.ajax({
                    url: '<?= base_url('sibabad/kas/saveKas') ?>', // URL untuk POST
                    method: 'POST',
                    data: {
                        kas_name: kas_name,
                        kas_kode: kas_kode,
        ...
        $(document).on('click', '.btnDeleteKasMasuk', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    // Perform the delete action using AJAX
                    $.ajax({
                        url: `<?= base_url('sibabad/kas/deleteKas/') ?>${id_kas}`,
                        method: 'DELETE',
                        dataType: 'json',
                        success: function (response) {
                            if (response.success) {
...
<script>
    var kas_total = document.getElementById('kas_total');
    kas_total.addEventListener('keyup', function (e) {
        this.value = formatRupiah(this.value);
    });

    var kas_keluar_total = document.getElementById('kas_keluar_total');
    kas_keluar_total.addEventListener('keyup', function (e) {
        this.value = formatRupiah(this.value);
    });
...
<script>
    $(document).ready(function () {
        var table = $('#table_transaksi').DataTable({
            ...
            lengthChange: true, // Tampilkan pilihan jumlah data per halaman
            searching: true, // Sembunyikan fitur pencarian
            info: true, // Sembunyikan informasi jumlah data
            "lengthMenu": [
                [5, 10, 20],
                [5, 10, 20]
            ]
        ...
        var data = <?= $transaksi_harian ?>;
        populateTable(data);

        document.getElementById('filter-tanggal').addEventListener('change', applyFilter);

        $('.wrapper').on('click', '#cetakpdf', function () {
            var selectedFilter = document.getElementById("filter-tanggal").value;
            var url = `<?= base_url('sibabad/pdf/transaksipdf') ?>?filter=${selectedFilter}`;
        ...
        function applyFilter() {
            ...
            // Send an AJAX request to the controller with the selected filter
            $.ajax({
                type: 'GET',
                url: '<?= base_url('sibabad/sale/transaksi_harian') ?>',
                data: {
                    filter: selectedFilter
                },
                success: function (respon) {
                    // Clear existing rows and repopulate the table
                    table.clear().draw();
                    populateTable(respon);
                },
                error: function () {
